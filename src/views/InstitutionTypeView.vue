  ...
  <div class="page-container">
    <div class="page-banner">
      <div class="breadcrumb">
        ...
        <span>Tipuri de instituții</span>
      </div>
    </div>

    <div class="page-content">
      <div class="content-header">
        <h1 class="page-title">Tipuri de instituții</h1>
        <button v-if="PERMISIONS.create" class="btn btn-primary" @click="openAdd">
          <span class="btn-icon">+</span> Adaugă tip de instituție
        </button>
      </div>
        ...
        <table class="data-table">
          <thead>
            ...
            <tr class="table-filter-row">
              ...
              <th>
                <div class="f-input">
                  <svg class="f-ico" width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg><input v-model="filterName" class="f-field" @input="currentPage = 1" /></div>
              </th>
              <th v-if="PERMISIONS.edit || PERMISIONS.delete">
                <button class="btn-clear-filters" @click="filterName = ''" title="Șterge filtrele"><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg></button>
              </th>
            </tr>
          </thead>
          <tbody>
    ...
    <div v-if="modal" class="modal-overlay" @click.self="closeModal">
      <div class="modal">
        <div class="modal-header">
          <h2 class="modal-title">{{ modal.mode === 'add' ? 'Adaugă' : 'Modifică' }} tip de instituție</h2>
        ...
        <div class="modal-body">
          ...
          <div class="form-group">
            <label class="form-label">Numele tipului de instituție <span class="required">*</span></label>
            <input v-model="modal.form.name" class="form-input"
              :class="{ 'form-input--error': formErrors.name }" />
            <span v-if="formErrors.name" class="field-error">Câmp obligatoriu</span>
          </div>

          <!-- Multi-select Tipuri de structuri organizatorice -->
          <div class="form-group">
            <label class="form-label">
              Tipuri de structuri organizatorice
              <span class="required">*</span>
              <span class="label-hint">(max 10)</span>
            </label>

            <!-- Dropdown de selecție -->
            <div class="multiselect-wrapper" :class="{ 'multiselect-wrapper--error': formErrors.orgTypes }">
              <div class="multiselect-selected" v-if="modal.form.organizationalStructureType.length > 0">
                <span
                  v-for="item in modal.form.organizationalStructureType"
                  :key="item.id"
                  class="multiselect-tag"
                >
                  {{ item.name }}
                  <button class="multiselect-tag-remove" @click="removeOrgType(item.id)">×</button>
                </span>
              </div>
              <div v-else class="multiselect-placeholder">Selectează tipuri de structuri...</div>

              <div class="multiselect-dropdown" v-if="availableOrgTypes.length > 0">
                <div
                  v-for="opt in availableOrgTypes"
                  :key="opt.id"
                  class="multiselect-option"
                  :class="{ 'multiselect-option--disabled': modal.form.organizationalStructureType.length >= 10 }"
                  @click="addOrgType(opt)"
                >
                  {{ opt.name }}
                </div>
                <div v-if="modal.form.organizationalStructureType.length >= 10" class="multiselect-limit">
                  Limita de 10 tipuri atinsă
                </div>
              </div>
              <div v-else-if="orgTypesLoading" class="multiselect-loading">Se încarcă...</div>
              <div v-else class="multiselect-empty">Toate tipurile au fost selectate</div>
            </div>
            <span v-if="formErrors.orgTypes" class="field-error">Selectați cel puțin un tip de structură</span>
          </div>
        ...
        <div class="modal-footer">
          ...
          <button class="btn btn-primary" @click="saveModal" :disabled="saving">
            ...
            {{ modal.mode === 'add' ? 'Adaugă' : 'Modifică' }}
          </button>
        </div>
      </div>
    </div>
    ...
    <div v-if="deleteTarget" class="modal-overlay" @click.self="deleteTarget = null">
      <div class="modal modal-sm">
        <div class="modal-header">
          <h2 class="modal-title">Șterge tip de instituție</h2>
        ...
        <div class="modal-body">
          <p class="delete-msg">Ești sigur că dorești să ștergi <strong>{{ deleteTarget.name }}</strong>?</p>
...
<script>
import { INSTITUTION_TYPE, ORGANIZATIONAL_STRUCTURE_TYPE } from '@/api.js'
...
export default {
  ...
  data() {
    return {
      rows: [], allOrgTypes: [], orgTypesLoading: false,
  ...
  computed: {
    filteredRows() {
      if (!this.filterName) return this.rows
      return this.rows.filter(r => (r.name || '').toLowerCase().includes(this.filterName.toLowerCase()))
    },
    paginatedRows() {
    ...
    // Tipurile de structuri disponibile = toate minus cele deja selectate
    availableOrgTypes() {
      if (!this.modal) return []
      const selectedIds = this.modal.form.organizationalStructureType.map(x => x.id)
      return this.allOrgTypes.filter(t => !selectedIds.includes(t.id))
    }
  },
  methods: {
    async loadData() {
      this.loading = true
      try {
        const res = await INSTITUTION_TYPE.get(1, 1000, '')
    ...
    async loadOrgTypes() {
      ...
      try {
        const res = await ORGANIZATIONAL_STRUCTURE_TYPE.get(1, 500, '')
        const data = await res.json()
        if (Array.isArray(data?.data?.result)) this.allOrgTypes = data.data.result
      } catch(e) { console.error(e) }
    ...
    openAdd() {
      this.modal = {
        mode: 'add',
        form: { name: '', organizationalStructureType: [] },
        originalId: null
      }
      this.formErrors = {}
      if (!this.allOrgTypes.length) this.loadOrgTypes()
    },

    openEdit(row) {
      this.activeMenu = null
      // organizationalStructureType vine din API ca array de obiecte { id, name }
      const orgTypes = Array.isArray(row.organizationalStructureType)
        ? row.organizationalStructureType
        : []
      this.modal = {
        mode: 'edit',
        form: { name: row.name || '', organizationalStructureType: [...orgTypes] },
        originalId: row.id
      }
      this.formErrors = {}
      if (!this.allOrgTypes.length) this.loadOrgTypes()
    ...
    addOrgType(opt) {
      if (this.modal.form.organizationalStructureType.length >= 10) return
      this.modal.form.organizationalStructureType.push(opt)
    },

    removeOrgType(id) {
      this.modal.form.organizationalStructureType =
        this.modal.form.organizationalStructureType.filter(x => x.id !== id)
    },

    validateForm() {
      this.formErrors = {}
      let valid = true
      if (!this.modal.form.name.trim()) { this.formErrors.name = true; valid = false }
      // La editare, structurile organizatorice sunt obligatorii
      if (this.modal.mode === 'edit' && this.modal.form.organizationalStructureType.length === 0) {
        this.formErrors.orgTypes = true; valid = false
      }
      return valid
    },
    ...
    async saveModal() {
      ...
      try {
        const sendData = {
          ...
          organizationalStructureType: this.modal.form.organizationalStructureType
        }
        if (this.modal.mode === 'add') {
          await INSTITUTION_TYPE.add(sendData)
        } else {
          await INSTITUTION_TYPE.edit(this.modal.originalId, sendData)
        }
        this.closeModal()
        await this.loadData()
      } catch(e) { console.error(e) }
    ...
    async executeDelete() {
      ...
      this.deleting = true
      try {
        await INSTITUTION_TYPE.delete(this.deleteTarget.id)
...
.modal { background: white; border-radius: 10px; width: 520px; max-width: 96vw; box-shadow: 0 8px 40px rgba(0,0,0,0.18); animation: fadeIn 0.18s ease; }
...
.form-label { font-size: 0.82rem; font-weight: 500; color: #444; display: flex; align-items: center; gap: 6px; }
.required { color: #e53935; }
.label-hint { font-size: 0.75rem; color: #999; font-weight: 400; }
...
.form-input:focus { outline: none; border-color: #1565C0; box-shadow: 0 0 0 3px rgba(21,101,192,0.08); }
.form-input--error { border-color: #e53935 !important; }
.field-error { font-size: 0.75rem; color: #e53935; }

/* Multi-select custom */
.multiselect-wrapper {
  border: 1.5px solid #ddd; border-radius: 6px; padding: 8px;
  min-height: 42px; display: flex; flex-direction: column; gap: 8px;
}
.multiselect-wrapper--error { border-color: #e53935; }
.multiselect-selected { display: flex; flex-wrap: wrap; gap: 6px; }
.multiselect-tag {
  display: inline-flex; align-items: center; gap: 4px;
  background: #E3F2FD; color: #1565C0; border-radius: 4px;
  padding: 3px 8px; font-size: 0.8rem; font-weight: 500;
}
.multiselect-tag-remove {
  background: none; border: none; cursor: pointer; color: #1565C0;
  font-size: 1rem; line-height: 1; padding: 0; margin-left: 2px;
}
.multiselect-tag-remove:hover { color: #e53935; }
.multiselect-placeholder { color: #aaa; font-size: 0.875rem; }
...
.multiselect-option {
  padding: 6px 8px; border-radius: 4px; cursor: pointer; font-size: 0.85rem; color: #333;
}
.multiselect-option:hover { background: #f0f4ff; color: #1565C0; }
.multiselect-option--disabled { opacity: 0.4; pointer-events: none; }
.multiselect-limit { font-size: 0.78rem; color: #f57c00; padding: 4px 8px; font-style: italic; }
.multiselect-loading, .multiselect-empty { font-size: 0.82rem; color: #aaa; padding: 4px 8px; font-style: italic; }
