  ...
  <div class="page">
    <div class="page-header">
      <div class="breadcrumb">
        ...
        <span class="breadcrumb-sep">/</span>
        <span class="breadcrumb-current">Procese verbale</span>
      </div>
      <h1 class="page__title">Procese verbale de evaluare a ofertelor</h1>
    </div>
        ...
        <table class="data-table">
          <thead>
            <tr>
              ...
              <th>Modalitate solicitare</th>
              <th>Modalitate primire</th>
              <th>Nr. furnizori</th>
              <th>Status</th>
              <th></th>
            </tr>
          ...
          <tbody>
            <tr v-if="!rows.length">
              <td colspan="8" class="empty-cell">Nu există procese verbale.</td>
            </tr>
            <tr v-for="(row, i) in rows" :key="row.id" class="tr-data">
              <td>{{ i + 1 }}</td>
              <td>{{ formatDate(row.verbalProcessDate) }}</td>
              <td>{{ row.attributionCriteria || '-' }}</td>
              <td>{{ row.requestModality || '-' }}</td>
              <td>{{ row.receiveModality || '-' }}</td>
              <td>{{ row.verbalProcessInstitutionProvider?.length || 0 }}</td>
              <td>
    ...
    <div v-if="deleteModal.open" class="modal-overlay" @click.self="deleteModal.open = false">
      <div class="modal-box">
        <h3 class="modal-title">Ștergere proces verbal</h3>
        <p>Ești sigur că vrei să ștergi acest proces verbal?</p>
...
<script>
import { VERBAL_PROCESS_GET, VERBAL_PROCESS_DELETE, VERBAL_PROCESS_GET_PDF, VERBAL_PROCESS_REQUEST_APPROVE, VERBAL_PROCESS_CHECK_APPROVED } from '@/api.js'
...
export default {
  ...
  methods: {
    ...
    async loadData() {
      this.loading = true
      try {
        const res = await VERBAL_PROCESS_GET(1, 500, `eq=paapCentralizationId,${this.centralizationId}`)
    ...
    async viewPdf(row) {
      try { const res = await VERBAL_PROCESS_GET_PDF(row.id); const blob = await res.blob(); window.open(URL.createObjectURL(blob), '_blank') } catch(e) { alert('PDF eșuat.') }
    },
    async requestApprove(row) {
      try { const res = await VERBAL_PROCESS_REQUEST_APPROVE(row.id); const d = await res.json(); if (d?.data?.result?.url) window.open(d.data.result.url, '_blank'); await this.loadData() } catch(e) { console.error(e) }
    },
    async checkApproved(row) { try { await VERBAL_PROCESS_CHECK_APPROVED(row.id); await this.loadData() } catch(e) { console.error(e) } },
    confirmDelete(row) { this.deleteModal = { open: true, row } },
    async doDelete() {
      this.saving = true
      try { await VERBAL_PROCESS_DELETE(this.deleteModal.row.id); this.deleteModal.open = false; await this.loadData() } catch(e) { console.error(e) }
      this.saving = false
    },
