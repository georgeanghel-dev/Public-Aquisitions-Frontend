  ...
  <div class="page-container">
    <div class="page-banner">
      <div class="breadcrumb">
        ...
        <span>Obiective instituție</span>
      </div>
    </div>

    <div class="page-content">
      <div class="content-header">
        <h1 class="page-title">Obiective instituție</h1>
        <button v-if="PERMISIONS.create" class="btn btn-primary" @click="openAdd">
          <span class="btn-icon">+</span> Adaugă obiectiv
        </button>
      </div>
        ...
        <table class="data-table">
            ...
            <tr class="table-filter-row">
              ...
              <th>
                <div class="filter-input date-filter">
                  <input type="date" v-model="filters.createdFrom" class="filter-field" @input="onFilter" />
                  <input type="date" v-model="filters.createdTo" class="filter-field" @input="onFilter" />
                </div>
              </th>
              <th></th>
              <th>
          ...
          <tbody>
            ...
            >
              ...
              <td>{{ row.disabledDate ? formatDate(row.disabledDate) : '-' }}</td>
              <td class="col-status">
            ...
            <tr v-if="filteredRows.length === 0">
              <td :colspan="PERMISIONS.edit || PERMISIONS.delete ? 6 : 5" class="empty-row">
    ...
    <div v-if="modal" class="modal-overlay" @click.self="closeModal">
      <div class="modal">
        <div class="modal-header">
          <h2 class="modal-title">{{ modal.mode === 'add' ? 'Adaugă' : 'Modifică' }} obiectiv</h2>
          <button class="btn-close" @click="closeModal">✕</button>
        </div>
        <div class="modal-body">
          <div class="form-group">
            <label class="form-label">Obiectiv <span class="required">*</span></label>
            <textarea
              v-model="modal.form.name"
              class="form-textarea"
              rows="5"
              :class="{ 'form-input--error': formErrors.name }"
              placeholder="Introduceți obiectivul instituției..."
            ></textarea>
    ...
    <div v-if="statusTarget" class="modal-overlay" @click.self="statusTarget = null">
      <div class="modal modal-sm">
        <div class="modal-header">
          <h2 class="modal-title">{{ statusTarget.activate ? 'Activează' : 'Dezactivează' }} obiectiv</h2>
          <button class="btn-close" @click="statusTarget = null">✕</button>
        </div>
        <div class="modal-body">
          <p class="delete-msg">
            Ești sigur că vrei să {{ statusTarget.activate ? 'activezi' : 'dezactivezi' }} obiectivul:
          </p>
          <p class="target-name">{{ statusTarget.row.name }}</p>
        </div>
        <div class="modal-footer">
    ...
    <div v-if="deleteTarget" class="modal-overlay" @click.self="deleteTarget = null">
      <div class="modal modal-sm">
        <div class="modal-header">
          <h2 class="modal-title">Șterge obiectiv</h2>
          <button class="btn-close" @click="deleteTarget = null">✕</button>
        </div>
        <div class="modal-body">
          <p class="delete-msg">Ești sigur că dorești să ștergi obiectivul selectat?</p>
...
import PermisionsMixin from '@/mixins/institutionObiectivePermMixin.js'
...
export default {
  ...
  data() {
    return {
      ...
      filters: { name: '', createdFrom: '', createdTo: '', isActive: '' },
  ...
  computed: {
    filteredRows() {
      return this.rows.filter(r => {
        ...
        if (this.filters.createdFrom && new Date(r.createdAt) < new Date(this.filters.createdFrom)) return false
        if (this.filters.createdTo && new Date(r.createdAt) > new Date(this.filters.createdTo + 'T23:59:59')) return false
        return true
      })
    },
    paginatedRows() {
  ...
  methods: {
    async loadData() {
      this.loading = true
      try {
        const res = await INSTITUTION_OBJECTIVES.get(1, 1000, '')
    ...
    clearFilters() { this.filters = { name: '', createdFrom: '', createdTo: '', isActive: '' } },
    toggleMenu(id) { this.activeMenu = this.activeMenu === id ? null : id },

    openAdd() {
      this.modal = { mode: 'add', form: { name: '' }, originalId: null }
      this.formErrors = {}
    },
    openEdit(row) {
      this.activeMenu = null
      this.modal = { mode: 'edit', form: { name: row.name || '' }, originalId: row.id }
      this.formErrors = {}
    },
    ...
    async saveModal() {
      this.formErrors = {}
      if (!this.modal.form.name.trim()) { this.formErrors.name = true; return }
      this.saving = true
      try {
        if (this.modal.mode === 'add') {
          await INSTITUTION_OBJECTIVES.add({ name: this.modal.form.name })
        } else {
          await INSTITUTION_OBJECTIVES.edit(this.modal.originalId, { name: this.modal.form.name })
        }
        this.closeModal()
        await this.loadData()
      } catch(e) { console.error(e) }
    ...
    async executeStatus() {
      ...
      this.saving = true
      try {
        await INSTITUTION_OBJECTIVES.changeStatus(this.statusTarget.row.id, {
          isActive: this.statusTarget.activate,
          disabledDate: this.statusTarget.activate ? null : new Date()
        })
    ...
    async executeDelete() {
      ...
      this.deleting = true
      try {
        await INSTITUTION_OBJECTIVES.delete(this.deleteTarget.id)
...
.date-filter { flex-direction: column; padding: 4px; gap: 3px; }
...
.col-name { white-space: normal; word-break: break-word; max-width: 400px; }
...
.actions-dropdown { position: absolute; right: 0; top: 100%; z-index: 100; background: white; border-radius: 6px; min-width: 150px; box-shadow: 0 4px 16px rgba(0,0,0,0.12); border: 1px solid #e8e8e8; overflow: hidden; }
...
.form-textarea { border: 1.5px solid #ddd; border-radius: 6px; padding: 0.5rem 0.75rem; font-size: 0.875rem; font-family: 'Roboto', sans-serif; color: #333; resize: vertical; }
.form-textarea:focus { outline: none; border-color: #1565C0; box-shadow: 0 0 0 3px rgba(21,101,192,0.08); }
...
.target-name { font-size: 0.875rem; font-weight: 500; color: #1565C0; padding: 8px 12px; background: #f0f4ff; border-radius: 6px; margin: 0; }
.delete-warning { font-size: 0.82rem; color: #e53935; margin: 0.5rem 0 0; }
